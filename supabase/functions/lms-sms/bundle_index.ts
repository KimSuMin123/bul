...
export function renderMessage(job: any, siteUrl = '') {
    ...
    switch (job.kind) {
        case 'enrollment_student': return `[세화불학원] ${name}님, ${courseTitle} 수강 신청이 접수되었습니다. 수강료 ${Number(price).toLocaleString('ko-KR')}원. 농협 301-0264-3664-41 ([사] 세화불학원). ${APPROVAL_NOTICE}`;
    ...
    return async (req: Request) => {
        ...
        if (input.action === 'test_send') {
            ...
            const defaultMessages = [
                `[세화불학원] 김세현님, 불교입문 강좌 수강 신청이 접수되었습니다. 수강료 50,000원. 농협 301-0264-3664-41 ([사] 세화불학원). ${APPROVAL_NOTICE}`,
