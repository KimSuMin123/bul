...
const names = ['Courses','Lectures','Enrollments','Payments','Progress','Certificates','QAPosts','ExamAttempts','DonationReceipts'];
...
try {
  ...
  await check('failed payment never starts sequential fallback or changes the ledger', async () => {
    const rejected = await page.evaluate(async () => {
      ...
      try { await course.recordPayment({userId:'student-a',courseId:'c1',amount:100,manager:'staff',methodMemo:'cash',withDonationReceipt:true,studentPhone:'01011112222'}); return false; } catch { return true; }
    });
    await flush();
    assert.equal(rejected, true);
    assert.equal(await page.evaluate(() => course.payments.length), 0);
    assert.equal(await page.evaluate(() => course.donationReceipts.length), 0);
    assert.equal(await page.evaluate(() => fixture.calls.some(([name]) => ['insertPayment','upsertEnrollment'].includes(name))), false);
  });
  await check('donation ledger preserves failed reads, accepts empty reads and clears on account change', async () => {
    await page.evaluate(() => {
      fixture.data.DonationReceipts = [{id:'receipt-1',phone:'010-1111-2222',totalAmount:100}];
      setTestUser({id:'admin',role:'admin'});
    });
    await page.waitForFunction(() => course.donationReceipts.length === 1 && !course.loading);
    await page.evaluate(async () => { fixture.fail.getDonationReceipts = true; await course.refreshData(); });
    await flush();
    assert.equal(await page.evaluate(() => course.donationReceipts[0].id), 'receipt-1');
    assert.equal(await page.evaluate(() => Boolean(course.error)), true);
    await page.evaluate(async () => {
      fixture.fail.getDonationReceipts = false; fixture.data.DonationReceipts = []; await course.refreshData();
    });
    await flush();
    assert.equal(await page.evaluate(() => course.donationReceipts.length), 0);
    await page.evaluate(async () => {
      fixture.data.DonationReceipts = [{id:'receipt-2'}]; await course.refreshData();
    });
    await page.waitForFunction(() => course.donationReceipts.length === 1);
    await page.evaluate(() => { fixture.calls = []; setTestUser({id:'student-a',role:'student'}); });
    await page.waitForFunction(() => !course.loading && course.donationReceipts.length === 0);
    assert.equal(await page.evaluate(() => fixture.calls.some(([name]) => name === 'getDonationReceipts')), false);
  });
