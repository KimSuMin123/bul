...
export default function CertificateModal({ certificate: rawCert, onClose }) {
  ...
  useLayoutEffect(() => {
    ...
    const fit = () => {
      ...
      const byWidth = (window.innerWidth - (mobile ? 16 : 40)) / shell.offsetWidth;
      const byHeight = mobile ? 1 : (window.innerHeight - 40) / shell.offsetHeight;
      shell.style.zoom = String(Math.min(1, byWidth, byHeight));
    };
    fit();
  ...
  return <div className="cert-modal-backdrop" onClick={onClose}>
    <div ref={shellRef} className="cert-modal-shell" role="dialog" aria-modal="true" aria-labelledby="certificate-title" onClick={event => event.stopPropagation()}>
      ...
      <article className="cert-paper" id="printable-certificate" aria-label={`${cert.studentName || ''} 자격증`}>
        ...
        <footer className="cert-footer"><p className="cert-date">{dateText(cert.issuedAt)}</p><div className="cert-issuer"><strong>{cert.issuingOrg || '[사] 세화불학원'} {cert.representative || '이사장'}</strong><SealGraphic size={96} /></div><p className="cert-office">{cert.regOffice || ''}</p><p className="cert-check">발급고유코드 {cert.certNo || '—'} · 학적식별번호 {cert.memberNo || '—'}</p><p className="cert-legal">본 자격증은 자격기본법 제17조 제2항에 따른 등록민간자격입니다. 세화불학원 온라인 학사관리시스템에서 진위를 확인할 수 있습니다.</p></footer>
      </article>
    </div>
  </div>;
}
